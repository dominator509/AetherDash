...
fn gen_adversarial_goldens() {
    ...
    let t = ts();
    let ordering_entries: Vec<Entry> = vec![entry(
        "order_multi_key_venue_ref",
        "Order",
        serde_json::to_value(aether_core::order::Order {
            order_id: ulid(),
    ...
    let entries: Vec<Entry> = vec![
        entry(
            "explicit_null",
            "Money",
            serde_json::to_value(Money::new(Decimal::new(100, 2), "USD")).unwrap(),
        ),
        entry(
            "omitted_optional",
            "Quote",
            serde_json::to_value(aether_core::quote::Quote {
                market: mk("kalshi", "BTC-75"),
                bid: Some(Decimal::new(65, 2)),
                ask: None,
                mid: None,
    ...
    let empty_entries: Vec<Entry> = vec![
        ...
        entry(
            "empty_risk_reasons",
            "RiskVerdict",
            serde_json::to_value(aether_core::order::RiskVerdict {
                intent_id: ulid(),
                verdict: aether_core::order::RiskVerdictStatus::Allow,
                reasons: vec![],
                ts: t,
        ...
        entry(
            "empty_caps_maps",
            "CapsSnapshot",
            serde_json::to_value(aether_core::order::CapsSnapshot {
                version: ulid(),
                per_order_max: Money::new(Decimal::new(50000, 2), "USD"),
                daily_max: Money::new(Decimal::new(100000, 2), "USD"),
