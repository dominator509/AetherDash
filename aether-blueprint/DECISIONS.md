...
---
## ADR-0011: Centralize authorization and pin authentication primitives
- **Context:** EP-401 must produce identical, deterministic authorization decisions at the gateway, router, MCP, and Guardian boundaries. Password hashing and TOTP are security-sensitive execution dependencies, while opaque bearer tokens still need indexed database lookup.
- **Decision:** `crates/aether-authz` is the Rust policy authority and exposes a closed action/tier matrix plus fail-closed audited enforcement adapters. Local passwords use Argon2id v1.3 with RFC 9106's second recommended profile (`m=65536 KiB`, `t=3`, `p=4`, 16-byte random salt, 32-byte output). TOTP uses RFC 6238 HMAC-SHA1 interoperability parameters (6 digits, 30-second step, +/-1 step). Session and step-up credentials are random 256-bit opaque values stored only as SHA-256 lookup hashes. The Python MCP boundary mirrors the crate's stable scope names and verdict ordering; execution services never import Python.
- **Alternatives:** Per-service ad-hoc tier checks (rejected: policy drift defeats defense in depth); Argon2 for random bearer-token lookup (rejected: bearer tokens already have 256 bits of entropy and indexed lookup must remain practical); storing TOTP secrets in Postgres (rejected: conflicts with the key-material isolation rule, so Postgres stores only a credential-store reference).
- **Consequences:** Argon2 adds a vetted cryptographic dependency and its intentionally expensive test increases authz test time. Services must load current grants per request, persist/transport every returned audit record, and fail closed if audit emission fails. TOTP secret provisioning remains operator/keychain-owned rather than a database migration concern.
- **Status:** Accepted. **Date:** 2026-07-14. **Owner:** Operator.
