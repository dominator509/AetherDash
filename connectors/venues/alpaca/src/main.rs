//! AETHER Terminal -- Alpaca venue adapter binary.
//!
//! Runs a Tonic gRPC server implementing `aether.venue.v1.VenueAdapter` with:
//!
//! - `list_markets` / `get_market`  (M2 -- real Alpaca REST calls)
//! - `stream_ticks` (authenticated WebSocket stream)
//! - `stream_book` (`capability_missing`; IEX has no depth feed)
//! - `submit_order` / `cancel_order` (paper-only V2 orders)
//! - `get_balances`                  (paper account)
//! - `health`                        (implemented)
...
//!
//! ```text
//! AETHER_VENUE__ALPACA_KEY_ID=ak_test                         \
//! AETHER_VENUE__ALPACA_SECRET=sk_test                         \
//! AETHER_VENUE__ALPACA_BASE_URL=https://paper-api.alpaca.markets \
//! AETHER_VENUE__ALPACA_DATA_URL=https://data.alpaca.markets     \
//! cargo run -p aether-venue-alpaca
//! ```
...
/// gRPC service implementation for Alpaca.
pub struct AlpacaVenueAdapter {
    client: client::AlpacaClient,
    orders: orders::AlpacaOrders,
    stream: Arc<ws::AlpacaStream>,
    last_tick_ms: Arc<AtomicI64>,
    quarantine_producer: Arc<BreakerProducer<KafkaProducer>>,
    quarantine_storage: Arc<QuarantineStorage>,
}

impl std::fmt::Debug for AlpacaVenueAdapter {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.debug_struct("AlpacaVenueAdapter").finish_non_exhaustive()
    }
}

impl AlpacaVenueAdapter {
    /// Create a new adapter with the given Alpaca REST client and orders handle.
    pub fn new(
        client: client::AlpacaClient,
        orders: orders::AlpacaOrders,
        stream: ws::AlpacaStream,
...
#[tonic::async_trait]
impl VenueAdapter for AlpacaVenueAdapter {
    ...
    async fn list_markets(
        ...
        let raw_assets = match self.client.list_assets(Some("active"), Some("us_equity")).await {
            Ok(assets) => assets,
            Err(error) => {
                if let Some(raw) = error.raw_payload() {
                    ...
                    .map_err(|quarantine_error| {
                        Status::internal(format!(
                            "asset response was malformed and quarantine failed: {quarantine_error}"
                        ))
                    })?;
                }
                return Err(Status::unavailable(format!("failed to fetch assets: {error}")));
            }
        };
        ...
        tokio::spawn(async move {
            for raw in raw_assets {
                ...
                match normalize::normalize_asset(raw) {
                    Ok(domain_market) => {
                        ...
                        .await
                        {
                            let _ = tx
                                .send(Err(Status::internal(format!(
                                    "failed to quarantine malformed asset: {quarantine_error}"
                                ))))
                                .await;
                            break;
                        }
                        tracing::warn!(error = %error, "skipping malformed asset");
                    }
                }
            }
        });
    ...
    async fn get_market(
        ...
        let symbol = key
            ...
            .ok_or_else(|| Status::invalid_argument("market key must be mkt:alpaca:{symbol}"))?
            .to_uppercase();

        let raw = match self.client.get_asset(&symbol).await {
            Ok(raw) => raw,
            Err(error) => {
                if let Some(raw) = error.raw_payload() {
                    Quarantine::publish(
                ...
                return Err(Status::unavailable(format!("failed to fetch asset: {error}")));
            }
        };

        let raw_bytes = serde_json::to_vec(&raw).unwrap_or_default();
        let domain = match normalize::normalize_asset(raw) {
            Ok(domain) => domain,
            Err(error) => {
                Quarantine::publish(
                    self.quarantine_producer.as_ref(),
                ...
                return Err(Status::internal("asset normalization failed; payload quarantined"));
            }
        };
    ...
    async fn stream_ticks(
        ...
        for key in request.into_inner().keys {
            let symbol = match parse_alpaca_market_key(&key.value) {
                Ok(s) => s.to_uppercase(),
        ...
        tokio::spawn(async move {
            ...
            if let Err(error) = stream
                ...
                .await
            {
                tracing::warn!(%error, "Alpaca tick stream ended");
            }
        });
        tokio::spawn(async move {
    ...
    async fn stream_book(
        &self,
        _request: Request<StreamBookRequest>,
    ) -> Result<Response<Self::StreamBookStream>, Status> {
        Err(capability_missing("books"))
    }
    ...
    async fn submit_order(
        ...
        let ack = self.orders.submit_order(&intent).await.map_err(map_order_error)?;
    ...
    async fn cancel_order(
        ...
        self.orders.cancel_order(&req.venue_ref).await.map_err(map_order_error)?;
    ...
    async fn get_balances(
        ...
        let domain = self.orders.get_balances().await.map_err(map_order_error)?;
...
fn capability_missing(capability: &str) -> Status {
    Status::failed_precondition(format!(
        "capability_missing: Alpaca paper pack does not declare '{capability}'"
    ))
}

fn map_order_error(error: orders::OrderError) -> Status {
    match error {
        orders::OrderError::DemoRequired | orders::OrderError::PaperIntentRequired => {
            Status::failed_precondition(error.to_string())
        }
        orders::OrderError::InvalidPrice(_) | orders::OrderError::Api(_) => {
            Status::invalid_argument(error.to_string())
        }
        orders::OrderError::Client(_) => Status::unavailable(error.to_string()),
    }
}
...
fn domain_market_to_proto(m: Market) -> Option<core_proto::Market> {
    ...
    let kind = match m.kind {
        aether_core::market::InstrumentKind::Equity => InstrumentKind::Equity as i32,
        aether_core::market::InstrumentKind::Option => InstrumentKind::Option as i32,
...
fn parse_alpaca_market_key(value: &str) -> Result<&str, &'static str> {
    value
        .strip_prefix("mkt:alpaca:")
        .filter(|s| !s.is_empty() && !s.contains(':'))
        .ok_or("market key must be mkt:alpaca:{symbol}")
}
...
fn proto_order_to_intent(proto: &core_proto::Order) -> Result<OrderIntent, String> {
    ...
    let market = proto
        ...
        .map(|m| {
            let symbol = m
                ...
                .strip_prefix("mkt:alpaca:")
                .filter(|s| !s.is_empty() && !s.contains(':'))
                .ok_or_else(|| "market key must be mkt:alpaca:{symbol}".to_string())?;
            MarketKey::new(&VenueId::new("alpaca").map_err(|e| e.to_string())?, symbol)
    ...
    // The proto Order message doesn't carry an explicit order_type;
    // infer it from presence of a limit price.
...
aether_feed_lag_ms{{venue=\"alpaca\"}} {lag}\n"
        );
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,aether_venue_alpaca=debug".into()),
        )
        .init();

    // -- Auth & client setup -- //
    let auth = auth::AlpacaAuth::from_env()?;
    let client = client::AlpacaClient::from_env(auth);

    // -- Orders client (separate AlpacaClient; reqwest::Client is pooled) -- //
    let orders_auth = auth::AlpacaAuth::from_env()?;
    let orders_client = client::AlpacaClient::from_env(orders_auth);
    let orders = orders::AlpacaOrders::new(orders_client);

    let stream_auth = auth::AlpacaAuth::from_env()?;
    let alpaca_stream = ws::AlpacaStream::from_env(stream_auth);
    let quarantine_producer = KafkaProducer::from_env()?;
    let quarantine_storage = QuarantineStorage::new_from_env()?;

    // -- gRPC adapter -- //
    let adapter = AlpacaVenueAdapter::new(
        client,
        orders,
        alpaca_stream,
        quarantine_producer,
    ...
    let grpc_addr = std::env::var("AETHER_VENUE__ALPACA_GRPC_ADDR")
        .unwrap_or_else(|_| "127.0.0.1:50057".to_string())
        .parse()?;
    let health_http_port: u16 = std::env::var("AETHER_VENUE__ALPACA_HEALTH_PORT")
        .ok()
    ...
    grpc_health_reporter.set_serving::<VenueAdapterServer<AlpacaVenueAdapter>>().await;
...
mod tests {
    ...
    fn metrics_exposes_feed_lag_from_the_same_tick_clock() {
        ...
        assert!(body.contains("aether_feed_lag_ms{venue=\"alpaca\"} 1250"));

        let (_, _, unknown) = health_http_response("GET /metrics HTTP/1.1\r\n", 0, 11_250);
        assert!(unknown.contains("aether_feed_lag_ms{venue=\"alpaca\"} NaN"));
    }
}
