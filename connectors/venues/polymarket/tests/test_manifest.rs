//! Manifest validation tests for the Polymarket venue pack.
//!
...
fn load_manifest() -> VenueManifest {
    ...
    toml::from_str(raw).expect("Polymarket venue.toml must be valid TOML")
}

#[test]
fn manifest_has_correct_slug() {
    let m = load_manifest();
    assert_eq!(m.slug, "polymarket");
}

#[test]
fn manifest_is_read_only() {
    let m = load_manifest();
    // Polymarket pack is read-only: no orders, no balances
...
#[test]
fn manifest_declares_binary_and_categorical_assets() {
    let m = load_manifest();
    assert!(m.asset_kinds.contains(&"binary_contract".to_string()));
    assert!(m.asset_kinds.contains(&"categorical_contract".to_string()));
}
...
fn manifest_has_valid_endpoints() {
    ...
    assert!(m.endpoints.sandbox.is_none(), "Polymarket does not publish a sandbox endpoint");
}
...
fn manifest_data_sources_are_official_api() {
    ...
    for source in [
        ...
        m.data_sources.resolution.as_ref(),
    ]
    .iter()
    .flatten()
    {
...
fn manifest_rate_limits_are_positive() {
    ...
    assert!(m.rate_limits.ws_subscriptions.unwrap_or(0) > 0);
}

#[test]
fn migration_0026_matches_pack_identity() {
    // The seed migration must use the same slug and capabilities as the manifest
    let m = load_manifest();
    let migration = include_str!("../../../../infra/migrations/0026_seed_polymarket_venue.up.sql");
    assert!(migration.contains("'polymarket'"), "migration must reference slug 'polymarket'");
