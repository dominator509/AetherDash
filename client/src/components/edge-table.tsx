  ...
  return (
    <div className={compact ? 'text-sm' : 'text-base'}>
      <table className="w-full border-collapse">
        <tbody>
          {rows.map(row => (
            <tr key={row.label} className={`border-b border-gray-100 ${row.isNet ? 'font-bold bg-gray-50' : ''}`}>
              <td className="py-1 px-2 text-gray-600">{row.label}</td>
              <td className={`py-1 px-2 text-right font-mono ${row.isNet ? 'text-blue-700' : row.isZero ? 'text-gray-400 italic' : 'text-gray-900'}`}>
